<div id="js_wsp_pwd_browser" class="tableview passbolt_controller_component_password_browser_controller passbolt_view_component_password_browser selection">
	<div class="tableview-header">
		<table>
			<thead>
			<tr>
				<th class="js_grid_column js_grid_column_multipleSelect selections s-cell">
					<!--input type="checkbox" value="all" id="checkbox-select-all" disabled="disabled"-->
					<div class="input checkbox"><label for="checkbox-select-all">select all</label></div>
				</th>
				<th class="js_grid_column js_grid_column_favorite selections s-cell">
					<a href="#"><i class="icon fav no-text"></i><span>fav</span></a>
				</th>
				<th class="js_grid_column js_grid_column_name m-cell">Resource</th>
				<th class="js_grid_column js_grid_column_username m-cell">Username</th>
				<th class="js_grid_column js_grid_column_secret m-cell password">Password</th>
				<th class="js_grid_column js_grid_column_uri l-cell">URI</th>
				<th class="js_grid_column js_grid_column_modified m-cell">Modified</th>
				<th class="js_grid_column js_grid_column_owner m-cell">Owner</th>
			</tr>
			</thead>
		</table>
	</div>
	<div class="tableview-content scroll">
		<table>
			<tbody>
			<tr id="50d77ff9-c358-4dfb-be34-1b63d7a10fce" class="selected">
				<td class="js_grid_column_multipleSelect selections s-cell">
					<div title="">
						<div id="multiple_select_checkbox_50d77ff9-c358-4dfb-be34-1b63d7a10fce"
								 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
							<div class="input checkbox">
								<input type="checkbox" value="50d77ff9-c358-4dfb-be34-1b63d7a10fce"
											 id="checkboxab9432e3-1ebe-37a4-c8ed-332408bf0a24" checked="checked">
								<label for="checkboxab9432e3-1ebe-37a4-c8ed-332408bf0a24"></label>
							</div>
						</div>
					</div>
				</td>
				<td class="js_grid_column_favorite selections s-cell">
					<div title="">
						<div id="favorite_50d77ff9-c358-4dfb-be34-1b63d7a10fce"
								 class="passbolt_controller_component_favorite_controller passbolt_view_component_favorite js_component ready">
							<a href="#" class="no-text">
								<i class="icon fav"></i>
								<span>fav</span>
							</a>
						</div>
					</div>
				</td>
				<td class="js_grid_column_name m-cell">
					<div title="salesforce account">
						salesforce account
					</div>
				</td>
				<td class="js_grid_column_username m-cell">
					<div title="passbolt">
						passbolt
					</div>
				</td>
				<td class="js_grid_column_secret m-cell password">
					<div title="">
						<div class="secret-copy"><a href="#copy_secret"><span>copy password to clipboard</span></a><pre>-----BEGIN PGP MESSAGE-----
Version: GnuPG v1.4.12 (GNU/Linux)


=30SP
-----END PGP MESSAGE-----
</pre>
						</div>
					</div>
				</td>
				<td class="js_grid_column_uri l-cell">
					<div title="https://salesforce.com">
						https://salesforce.com
					</div>
				</td>
				<td class="js_grid_column_modified m-cell">
					<div title="2 years ago">
						2 years ago
					</div>
				</td>
				<td class="js_grid_column_owner m-cell">
					<div title="anonymous@passbolt.com">
						anonymous@passbolt.com
					</div>
				</td>
			</tr>
			<tr id="50d77ff9-fdd8-4035-b7c6-1b63d7a10fce">

				<td class="js_grid_column_multipleSelect selections s-cell">
					<div title="">
						<div id="multiple_select_checkbox_50d77ff9-fdd8-4035-b7c6-1b63d7a10fce"
								 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
							<div class="input checkbox">
								<input type="checkbox" value="50d77ff9-fdd8-4035-b7c6-1b63d7a10fce"
											 id="checkbox35e0690a-ed1e-33c9-779a-8e6b94e1a5dc">
								<label for="checkbox35e0690a-ed1e-33c9-779a-8e6b94e1a5dc"></label>
							</div>
						</div>
					</div>
				</td>

				<td class="js_grid_column_favorite selections s-cell">
					<div title="">
						<div id="favorite_50d77ff9-fdd8-4035-b7c6-1b63d7a10fce"
								 class="passbolt_controller_component_favorite_controller passbolt_view_component_favorite js_component ready">
							<a href="#" class="no-text">
								<i class="icon fav"></i>
								<span>fav</span>
							</a>
						</div>
					</div>
				</td>

				<td class="js_grid_column_name m-cell">
					<div title="tetris license">
						tetris license
					</div>
				</td>

				<td class="js_grid_column_username m-cell">
					<div title="passbolt">
						passbolt
					</div>
				</td>

				<td class="js_grid_column_secret m-cell password">
					<div title="">
						<div class="secret-copy"><a href="#copy_secret"><span>copy password to clipboard</span></a><pre>-----BEGIN PGP MESSAGE-----
Version: GnuPG v1.4.12 (GNU/Linux)


9AaKzfwbSvV5
=JVBU
-----END PGP MESSAGE-----
</pre>
						</div>
					</div>
				</td>

				<td class="js_grid_column_uri l-cell">
					<div title="https://tetris.com">
						https://tetris.com
					</div>
				</td>

				<td class="js_grid_column_modified m-cell">
					<div title="2 years ago">
						2 years ago
					</div>
				</td>

				<td class="js_grid_column_owner m-cell">
					<div title="anonymous@passbolt.com">
						anonymous@passbolt.com
					</div>
				</td>

			</tr>
			<tr id="509bb871-5168-49d4-a676-fb098cebc04d">
				<td class="js_grid_column_multipleSelect selections s-cell">
					<div title="">
						<div id="multiple_select_checkbox_509bb871-5168-49d4-a676-fb098cebc04d"
								 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
							<div class="input checkbox">
								<input type="checkbox" value="509bb871-5168-49d4-a676-fb098cebc04d"
											 id="checkbox957d9757-fa82-597e-2d0f-8f112bb3e046">
								<label for="checkbox957d9757-fa82-597e-2d0f-8f112bb3e046"></label>
							</div>
						</div>
					</div>
				</td>
				<td class="js_grid_column_favorite selections s-cell">
					<div title="">
						<div id="favorite_509bb871-5168-49d4-a676-fb098cebc04d"
								 class="passbolt_controller_component_favorite_controller passbolt_view_component_favorite js_component ready">
							<a href="#" class="no-text">
								<i class="icon fav"></i>
								<span>fav</span>
							</a>
						</div>
					</div>
				</td>
				<td class="js_grid_column_name m-cell">
					<div title="facebook account">
						facebook account
					</div>
				</td>
				<td class="js_grid_column_username m-cell">
					<div title="passbolt">
						passbolt
					</div>
				</td>
				<td class="js_grid_column_secret m-cell password">
					<div title="">
						<div class="secret-copy"><a href="#copy_secret"><span>copy password to clipboard</span></a><pre>-----BEGIN PGP MESSAGE-----
Version: GnuPG v1.4.12 (GNU/Linux)


IGToRTYP
=UgZQ
-----END PGP MESSAGE-----
</pre>
						</div>
					</div>
				</td>
				<td class="js_grid_column_uri l-cell">
					<div title="https://facebook.com">
						https://facebook.com
					</div>
				</td>
				<td class="js_grid_column_modified m-cell">
					<div title="2 years ago">
						2 years ago
					</div>
				</td>
				<td class="js_grid_column_owner m-cell">
					<div title="ismail@passbolt.com">
						ismail@passbolt.com
					</div>
				</td>
			</tr>
			</tbody>
		</table>
	</div>
</div>