<!doctype html>
<html class="alpha version no-js" lang="en">
<head>
    <meta charset="utf-8">
    <title>Passbolt | Open source password manager for teams</title>
    <?php include('includes/meta/AA_meta.php'); ?>
    <?php include('includes/headers/AA_header_scripts.php'); ?>
</head>
<body>
<div id="container" class="page roadmap featured">
    <?php include('includes/headers/AA_header.php'); ?>
    <div class="page-row intro">
        <div class="grid grid-responsive-12">
            <div class="row">
                <div class="col12">
                <h1>Features & roadmap</h1>
                <p>
                    Here are all the next majors milestone for passbolt in the near future.
                    You can vote for new ideas and change requests in the community forum.
                </p>
                <a href="https://community.passbolt.com" class="button primary big">Discuss the roadmap!</a>
                </div>
            </div>
        </div>
    </div>
    <?php include('includes/public/AA_roadmap1.php'); ?>
    <?php include('includes/AA_signup.php'); ?>
    <?php include('includes/AN_footer.php'); ?>
</div>
</body>
</html>