<!doctype html>
<html class=" js websqldatabase draganddrop cssscrollbar" lang="en">
<head>
    <meta charset="utf-8">
    <title>Passbolt - The simple password management system</title>
    <!--
             ____                  __          ____
            / __ \____  _____ ____/ /_  ____  / / /_
           / /_/ / __ `/ ___/ ___/ __ \/ __ \/ / __/
          / ____/ /_/ (__  |__  ) /_/ / /_/ / / /_
         /_/    \__,_/____/____/_.___/\____/_/\__/

         The password management solution
         (c) 2014 passbolt.com

     -->
    <base href="../src/">
    <meta content="IE=edge,chrome=1" http-equiv="X-UA-Compatible">
    <link rel="stylesheet" type="text/css" href="css/main.css">
    <link rel="stylesheet" type="text/css" href="css/devel.css">
    <script src="js/modernizr.js"></script>
    <script type="application/javascript">
        var cakephpConfig = {
            app: {
                name: "Passbolt",
                punchline: "The simple password management system",
                copyright: "",
                title: "%s | Passbolt",
                version: {
                    number: "2.13.3",
                    name: "Sauvage",
                    song: "http://youtu.be/DaRG0ukxYqQ"
                },
                url: "http://passbolt.dev/",
                debug: "3"
            },
            user: {
                id: "533d37a0-bc80-4945-9b11-1663c0a895dc"
            },
            roles: {
                "user": "0208f57a-c5cd-11e1-a0c5-080027796c4c",
                "admin": "142c1188-c5cd-11e1-a0c5-080027796c4c"
            },
            image_storage: {
                public_path: "img/public"
            }
        };
    </script>
</head>
<body cz-shortcut-listen="true">
<!-- main -->
<div id="container" class="page">
<div class="mad_event_event_bus"></div>
<div id="js_app_controller"
     class="passbolt_controller_app_controller mad_view_view js_component ready">
<div id="js_app_loading_bar"
     class="update-loading-bar passbolt_controller_component_loading_bar_controller passbolt_view_component_loading_bar js_component ready">
    <div class="progress-bar">
        <span style="width: 0%;"></span>
    </div>
</div>
<div class="notification-container">
    <div id="js_app_notificator"
         class="notification col2_3 push_1 passbolt_controller_component_notification_controller">
    </div>
</div>
<div class="header first">
	<nav>
		<div class="primary navigation top">
			<ul id="js_app_navigation_left"
					class="left passbolt_controller_component_app_navigation_left_controller mad_view_component_tree menu ready">
				<li id="0cd2189d-84fd-597b-3970-66be2971d0d2" class="home ready">
					<div class="row">
						<div class="main-cell-wrapper">
							<div class="main-cell">
								<a href="../demo/LU_passwords.php"><span>home</span></a>
							</div>
						</div>
					</div>


				</li>
				<li id="31a2d90f-6c19-8475-0603-8bbb2e0130df" class="passwords ready">
					<div class="row">
						<div class="main-cell-wrapper">
							<div class="main-cell">
								<a href="../demo/LU_passwords.php"><span>passwords</span></a>
							</div>
						</div>
					</div>


				</li>
				<li id="067a31de-232c-32a0-8a6d-56c3bfdf83b0" class="users ready">
					<div class="row">
						<div class="main-cell-wrapper">
							<div class="main-cell">
								<a href="../demo/LU_users.php"><span>users</span></a>
							</div>
						</div>
					</div>


				</li>
			</ul>
			<ul id="js_app_navigation_right"
					class="right passbolt_controller_component_app_navigation_right_controller mad_view_component_tree menu ready">
				<li id="ea699358-a04d-179f-cc6c-64e7bd7f4254" class="logout ready">
					<div class="row">
						<div class="main-cell-wrapper">
							<div class="main-cell">
								<a href="../demo/AN_login.php"><span>logout</span></a>
							</div>
						</div>
					</div>


				</li>
			</ul>
		</div>
	</nav>
</div>
<div class="header second">
    <div class="col1">
        <div class="logo">
            <img src="img/logo/logo.png" alt="passbolt">
        </div>
    </div>
    <div class="col2 search-wrapper">
        <div id="js_app_filter"
             class="passbolt_controller_component_app_filter_controller passbolt_view_component_app_filter js_component ready">
            <form id="js_app_filter_form"
                  class="search mad_form_form_controller mad_view_form_form_view js_component ready">
                <!-- <ul id="js_filter_tags" class="tags"></ul> -->
                <div class="input search required">
                    <label for="js_app_filter_form">
                        Search</label>
                    <input id="js_app_filter_keywords"
                           class="required mad_form_element_textbox_controller mad_view_form_element_textbox_view js_component ready"
                           maxlength="50" type="search"
                           placeholder="search people">
                </div>
                <button value="search">
                    <i class="icon search"></i>
                    <span class="text visuallyhidden">search</span>
                </button>
            </form>
            <!-- <form id="js_filter_form">
                <ul id="js_filter_tags" class="tags"></ul>
                <fieldset>
                    <legend>
                        Please enter a search keyword
                    </legend>
                    <div class="input text required">
                        <label for="js_filter_keywords">
                            Search</label>
                        <input id="js_filter_keywords" class="required" maxlength="50" type="text"/>
                    </div>
                </fieldset>
                <span id="js_filter_reset" class="control reset">x</span>
                <div class="submit">
                    <input type="submit" />
                </div>
            </form> --></div>
    </div>
    <div class="col3 profile-wrapper">
        <div id="js_app_profile_dropdown"
             class="user profile dropdown passbolt_controller_component_profile_dropdown_controller mad_view_component_button_dropdown js_component ready">
            <div class="center-cell-wrapper">
                <div class="details center-cell">
                    <span class="name">Admin Istrator</span>
                    <span class="email">admin@passbolt.com</span>
                </div>
            </div>
            <div class="picture left-cell">
                <img src="img/avatar/user.png" alt="your picture">
            </div>
            <div class="more right-cell">
                <a href="#"><span>more</span></a>
            </div>
            <ul class="dropdown-content right mad_controller_component_menu_controller mad_view_component_tree menu ready"
                id="2b051246-09ea-4393-3de4-028f447982b2">
                <li id="e752f530-ba43-9e3e-25e2-3ea51352ca78" class="ready">
                    <div class="row">
                        <div class="main-cell-wrapper">
                            <div class="main-cell">
                                <a href="#"><span>my profile</span></a>
                            </div>
                        </div>
                    </div>


                </li>
                <li id="d4e0eff3-c6fa-1d6f-3670-0c39e5786cec" class="ready">
                    <div class="row">
                        <div class="main-cell-wrapper">
                            <div class="main-cell">
                                <a href="#"><span>manage or generate new keys</span></a>
                            </div>
                        </div>
                    </div>


                </li>
                <li id="948966d5-9663-ccd4-2477-6d7cfd50d67e"
                    class="separator-after ready">
                    <div class="row">
                        <div class="main-cell-wrapper">
                            <div class="main-cell">
                                <a href="#"><span>manage people</span></a>
                            </div>
                        </div>
                    </div>


                </li>
                <li id="e3d4181a-b5da-c64d-751d-77045bf041de" class="ready">
                    <div class="row">
                        <div class="main-cell-wrapper">
                            <div class="main-cell">
                                <a href="#"><span>logout</span></a>
                            </div>
                        </div>
                    </div>


                </li>
            </ul>
        </div>
    </div>
</div>
<div class="header third">
    <div class="col1 workspace-title-wrapper">
        <h1 class="workspace-title">Passwords</h1>
    </div>
    <div class="col2_3 actions-wrapper">
        <ul id="js_wsp_primary_menu"
            class="actions mad_controller_component_tab_controller mad_view_component_tab js_component ready">
            <div class="js_tabs_content tabs-content">
                <div id="js_passbolt_passwordWorkspaceMenu_controller"
                     class="passbolt_controller_component_password_workspace_menu_controller mad_view_view tab-content hidden"
                     style="display: none;">
                    <li>
                        <a id="js_wk_menu_creation_button" href="#"
                           class="button mad_controller_component_button_controller mad_view_view js_component ready">
                            <i class="icon create"></i>
                            <span>create</span>
                        </a>
                    </li>
                    <li>
                        <a id="js_wk_menu_edition_button" href="#"
                           class="button mad_controller_component_button_controller mad_view_view js_component disabled"
                           disabled="disabled">
                            <i class="icon edit"></i>
                            <span>edit</span>
                        </a>
                    </li>
                    <li>
                        <a id="js_wk_menu_deletion_button" href="#"
                           class="button mad_controller_component_button_controller mad_view_view js_component disabled"
                           disabled="disabled">
                            <i class="icon delete"></i>
                            <span>delete</span>
                        </a>
                    </li>
                    <li>
                        <a id="js_wk_menu_sharing_button" href="#"
                           class="button mad_controller_component_button_controller mad_view_view js_component disabled"
                           disabled="disabled">
                            <i class="icon share"></i>
                            <span>share</span>
                        </a>
                    </li>
                    <li>
                        <div class="dropdown">
                            <a id="js_wk_menu_more_button" href="#"
                               class="button mad_controller_component_button_dropdown_controller mad_view_component_button_dropdown js_component disabled"
                               disabled="disabled">
                                <span>more</span>
                                <i class="icon after arrowdown"></i>
                            </a>
                            <ul class="dropdown-content mad_controller_component_menu_controller mad_view_component_tree menu ready"
                                id="6b585e6e-c4cc-71a2-55c0-77c3df61a2a0">
                                <li id="47274697-b275-bfb1-f526-ef105c511293"
                                    class="todo ready">
                                    <div class="row">
                                        <div class="main-cell-wrapper">
                                            <div class="main-cell">
                                                <a href="#"><span>copy login to clipboard</span></a>
                                            </div>
                                        </div>
                                    </div>


                                </li>
                                <li id="4690b6fc-9bb1-7e65-183b-ae20986aa973"
                                    class="todo ready">
                                    <div class="row">
                                        <div class="main-cell-wrapper">
                                            <div class="main-cell">
                                                <a href="#"><span>copy password to clipboard</span></a>
                                            </div>
                                        </div>
                                    </div>


                                </li>
                            </ul>
                        </div>
                    </li>
                </div>
                <div id="js_passbolt_peopleWorkspaceMenu_controller"
                     class="passbolt_controller_component_people_workspace_menu_controller mad_view_view tab-content selected selection">

                    <li>
                        <a id="js_user_wk_menu_creation_button" href="#"
                           class="button mad_controller_component_button_controller mad_view_view js_component ready">
                            <i class="icon create"></i>
                            <span>create</span>
                        </a>
                    </li>
                    <li>
                        <a id="js_user_wk_menu_edition_button" href="#"
                           class="button mad_controller_component_button_controller mad_view_view js_component ready">
                            <i class="icon edit"></i>
                            <span>edit</span>
                        </a>
                    </li>
                    <li>
                        <a id="js_user_wk_menu_deletion_button" href="#"
                           class="button mad_controller_component_button_controller mad_view_view js_component ready">
                            <i class="icon delete"></i>
                            <span>delete</span>
                        </a>
                    </li>
                    <li>
                        <div class="dropdown" style="display: none">
                            <a id="js_user_wk_menu_more_button" href="#"
                               class="button mad_controller_component_button_dropdown_controller mad_view_component_button_dropdown js_component ready">
                                <span>more</span>
                                <i class="icon after arrowdown"></i>
                            </a>
                            <ul class="dropdown-content mad_controller_component_menu_controller mad_view_component_tree menu ready"
                                id="b1b8da93-4e0e-9a26-b5cd-567ed02878fb">
                                <li id="js_ppl_wk_remove_user_from_group"
                                    class="disabled">
                                    <div class="row">
                                        <div class="main-cell-wrapper">
                                            <div class="main-cell">
                                                <a href="#"><span>remove user from group</span></a>
                                            </div>
                                        </div>
                                    </div>


                                </li>


                            </ul>
                        </div>
                    </li>

                </div>
                <div id="js_passbolt_preferenceWorkspaceMenu_controller"
                     class="passbolt_controller_component_preference_workspace_menu_controller"></div>
            </div>
        </ul>
        <ul id="js_wsp_secondary_menu"
            class="actions secondary passbolt_controller_component_workspace_secondary_menu_controller mad_view_view js_component ready">
            <li>
                <a id="js_wk_secondary_menu_view_sidebar_button" href="#"
                   class="button toggle mad_controller_component_toggle_button_controller mad_view_view js_component ready">
                    <i class="icon layout eye big no-text"></i>
                    <span>view sidebar</span>
                </a>
            </li>
        </ul>
    </div>
</div>
<div id="js_app_panel_main"
     class="panel main mad_controller_component_tab_controller mad_view_component_tab js_component ready">
<div class="js_tabs_content tabs-content">
<div id="js_passbolt_passwordWorkspace_controller"
     class="passbolt_controller_password_workspace_controller mad_view_view tab-content hidden"
     style="display: none;">
<div class="js_workspace">
<div class="panel left">
    <div class="navigation first shortcuts">
        <ul id="js_wsp_pwd_rs_shortcuts"
            class="clearfix passbolt_controller_component_resource_shortcuts_controller mad_view_component_tree menu ready">
            <li id="de34df15-d627-ec21-b718-7d462a73d1e1"
                class="selected ready">
                <div class="row">
                    <div class="main-cell-wrapper">
                        <div class="main-cell">
                            <a href="#"><span>All items</span></a>
                        </div>
                    </div>
                </div>


            </li>
            <li id="c05bc2d7-b74d-06b0-320e-57dbafdfa3fd" class="ready">
                <div class="row">
                    <div class="main-cell-wrapper">
                        <div class="main-cell">
                            <a href="#"><span>Favorite</span></a>
                        </div>
                    </div>
                </div>


            </li>
            <li id="d5184d6d-4d76-a201-f7bd-4dc719aa9872" class="ready">
                <div class="row">
                    <div class="main-cell-wrapper">
                        <div class="main-cell">
                            <a href="#"><span>Recently modified</span></a>
                        </div>
                    </div>
                </div>


            </li>
            <li id="b40b88aa-6694-b131-a546-58d30a22bf0c" class="ready">
                <div class="row">
                    <div class="main-cell-wrapper">
                        <div class="main-cell">
                            <a href="#"><span>Shared with me</span></a>
                        </div>
                    </div>
                </div>


            </li>
            <li id="61f1301d-b568-6fec-d150-ef2a79a18afa" class="ready">
                <div class="row">
                    <div class="main-cell-wrapper">
                        <div class="main-cell">
                            <a href="#"><span>Items I own</span></a>
                        </div>
                    </div>
                </div>


            </li>
        </ul>
    </div>
    <div class="navigation last tree categories">
        <ul id="js_wsp_pwd_category_chooser">
        </ul>
    </div>
</div>
<div class="panel middle">
<div id="js_wsp_pwd_breadcrumb"
     class="breadcrumbs passbolt_controller_component_password_breadcrumb_controller mad_view_view js_component ready">
    <ul class="mad_controller_component_menu_controller mad_view_component_tree menu ready"
        id="34e2ab58-be5c-22ab-dbd9-d6c7effd87cc">
        <li id="f14e4945-9c09-9606-4581-4cb44077523c" class="ready">
            <div class="main-cell">
                <a href="#"><span>All items</span></a>
            </div>
        </li>
    </ul>
</div>
<div id="js_wsp_pwd_browser"
     class="tableview passbolt_controller_component_password_browser_controller passbolt_view_component_password_browser ready">
<div class="tableview-header">
    <table>
        <thead>
        <tr>

            <th class="js_grid_column js_grid_column_multipleSelect cell_multipleSelect selections cell_multipleSelect s-cell">
                <div class="input checkbox"><label for="checkbox-select-all">select
                    all</label></div>
            </th>

            <th class="js_grid_column js_grid_column_favorite selections cell_multipleSelect s-cell">
                <a href="#"> <i class="icon fav no-text"></i> <span>fav</span>
                </a>
            </th>

            <th class="js_grid_column js_grid_column_name m-cell">
                Avatar
            </th>

            <th class="js_grid_column js_grid_column_username m-cell">
                Username
            </th>

            <th class="js_grid_column js_grid_column_secret m-cell password">
                Password
            </th>

            <th class="js_grid_column js_grid_column_uri l-cell">
                URI
            </th>

            <th class="js_grid_column js_grid_column_modified m-cell">
                Modified
            </th>

            <th class="js_grid_column js_grid_column_owner m-cell">
                Owner
            </th>

            <th class="js_grid_column js_grid_column_copyLogin s-cell">

            </th>

            <th class="js_grid_column js_grid_column_copySecret s-cell">

            </th>

        </tr>
        </thead>
    </table>
</div>
<div class="tableview-content scroll">
<table>
<tbody>
<tr id="50d77ffd-5624-492c-842e-1b63d7a10fce">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_50d77ffd-5624-492c-842e-1b63d7a10fce"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="50d77ffd-5624-492c-842e-1b63d7a10fce"
                           id="checkbox1aaf6351-da25-f538-ea13-09a0232680fa">
                    <label for="checkbox1aaf6351-da25-f538-ea13-09a0232680fa"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_favorite selections cell_multipleSelect s-cell">
        <div title="">
            <div id="favorite_50d77ffd-5624-492c-842e-1b63d7a10fce"
                 class="passbolt_controller_component_favorite_controller passbolt_view_component_favorite js_component ready">
                <a href="#" class="no-text">
                    <i class="icon fav"></i>
                    <span>fav</span>
                </a>
            </div>
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="shared resource">
            shared resource
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="admin">
            admin
        </div>
    </td>

    <td class="js_grid_column_secret m-cell password">
        <div title="">
            <div class="secret-copy"><a href="#copy_secret"><span>copy password to clipboard</span></a><pre>-----BEGIN PGP MESSAGE-----
Version: GnuPG v1.4.12 (GNU/Linux)


=alLa
-----END PGP MESSAGE-----
</pre>
            </div>
        </div>
    </td>

    <td class="js_grid_column_uri l-cell">
        <div title="http://shared.resource.net/">
            http://shared.resource.net/
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="2 years ago">
            2 years ago
        </div>
    </td>

    <td class="js_grid_column_owner m-cell">
        <div title="anonymous@passbolt.com">
            anonymous@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_copyLogin s-cell">
        <div title="">

        </div>
    </td>

    <td class="js_grid_column_copySecret s-cell">
        <div title="">

        </div>
    </td>

</tr>
<tr id="50d77ff9-c358-4dfb-be34-1b63d7a10fce">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_50d77ff9-c358-4dfb-be34-1b63d7a10fce"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="50d77ff9-c358-4dfb-be34-1b63d7a10fce"
                           id="checkboxd117455d-757c-765d-6a48-7b802e3dae30">
                    <label for="checkboxd117455d-757c-765d-6a48-7b802e3dae30"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_favorite selections cell_multipleSelect s-cell">
        <div title="">
            <div id="favorite_50d77ff9-c358-4dfb-be34-1b63d7a10fce"
                 class="passbolt_controller_component_favorite_controller passbolt_view_component_favorite js_component ready">
                <a href="#" class="no-text">
                    <i class="icon fav"></i>
                    <span>fav</span>
                </a>
            </div>
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="salesforce account">
            salesforce account
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="passbolt">
            passbolt
        </div>
    </td>

    <td class="js_grid_column_secret m-cell password">
        <div title="">
            <div class="secret-copy"><a href="#copy_secret"><span>copy password to clipboard</span></a><pre>-----BEGIN PGP MESSAGE-----
Version: GnuPG v1.4.12 (GNU/Linux)


=YKV8
-----END PGP MESSAGE-----
</pre>
            </div>
        </div>
    </td>

    <td class="js_grid_column_uri l-cell">
        <div title="https://salesforce.com">
            https://salesforce.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="2 years ago">
            2 years ago
        </div>
    </td>

    <td class="js_grid_column_owner m-cell">
        <div title="anonymous@passbolt.com">
            anonymous@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_copyLogin s-cell">
        <div title="">

        </div>
    </td>

    <td class="js_grid_column_copySecret s-cell">
        <div title="">

        </div>
    </td>

</tr>
<tr id="50d77ff9-fdd8-4035-b7c6-1b63d7a10fce">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_50d77ff9-fdd8-4035-b7c6-1b63d7a10fce"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="50d77ff9-fdd8-4035-b7c6-1b63d7a10fce"
                           id="checkbox05588bbc-7440-99f1-2b89-5538fa0a43bd">
                    <label for="checkbox05588bbc-7440-99f1-2b89-5538fa0a43bd"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_favorite selections cell_multipleSelect s-cell">
        <div title="">
            <div id="favorite_50d77ff9-fdd8-4035-b7c6-1b63d7a10fce"
                 class="passbolt_controller_component_favorite_controller passbolt_view_component_favorite js_component ready">
                <a href="#" class="no-text">
                    <i class="icon fav"></i>
                    <span>fav</span>
                </a>
            </div>
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="tetris license">
            tetris license
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="passbolt">
            passbolt
        </div>
    </td>

    <td class="js_grid_column_secret m-cell password">
        <div title="">
            <div class="secret-copy"><a href="#copy_secret"><span>copy password to clipboard</span></a><pre>-----BEGIN PGP MESSAGE-----
Version: GnuPG v1.4.12 (GNU/Linux)


sZCiIV91Oajs
=Kqnu
-----END PGP MESSAGE-----
</pre>
            </div>
        </div>
    </td>

    <td class="js_grid_column_uri l-cell">
        <div title="https://tetris.com">
            https://tetris.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="2 years ago">
            2 years ago
        </div>
    </td>

    <td class="js_grid_column_owner m-cell">
        <div title="anonymous@passbolt.com">
            anonymous@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_copyLogin s-cell">
        <div title="">

        </div>
    </td>

    <td class="js_grid_column_copySecret s-cell">
        <div title="">

        </div>
    </td>

</tr>
<tr id="509bb871-5168-49d4-a676-fb098cebc04d">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_509bb871-5168-49d4-a676-fb098cebc04d"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="509bb871-5168-49d4-a676-fb098cebc04d"
                           id="checkbox0e0016a3-4fe4-b232-5376-51ba10e22398">
                    <label for="checkbox0e0016a3-4fe4-b232-5376-51ba10e22398"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_favorite selections cell_multipleSelect s-cell">
        <div title="">
            <div id="favorite_509bb871-5168-49d4-a676-fb098cebc04d"
                 class="passbolt_controller_component_favorite_controller passbolt_view_component_favorite js_component ready">
                <a href="#" class="no-text">
                    <i class="icon fav"></i>
                    <span>fav</span>
                </a>
            </div>
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="facebook account">
            facebook account
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="passbolt">
            passbolt
        </div>
    </td>

    <td class="js_grid_column_secret m-cell password">
        <div title="">
            <div class="secret-copy"><a href="#copy_secret"><span>copy password to clipboard</span></a><pre>-----BEGIN PGP MESSAGE-----
Version: GnuPG v1.4.12 (GNU/Linux)


lke7+Nwa
=ngpR
-----END PGP MESSAGE-----
</pre>
            </div>
        </div>
    </td>

    <td class="js_grid_column_uri l-cell">
        <div title="https://facebook.com">
            https://facebook.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="2 years ago">
            2 years ago
        </div>
    </td>

    <td class="js_grid_column_owner m-cell">
        <div title="ismail@passbolt.com">
            ismail@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_copyLogin s-cell">
        <div title="">

        </div>
    </td>

    <td class="js_grid_column_copySecret s-cell">
        <div title="">

        </div>
    </td>

</tr>
<tr id="509bb871-b964-48ab-94fe-fb098cebc04d">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_509bb871-b964-48ab-94fe-fb098cebc04d"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="509bb871-b964-48ab-94fe-fb098cebc04d"
                           id="checkboxdff54310-6e7d-ba29-5203-4281effea56f">
                    <label for="checkboxdff54310-6e7d-ba29-5203-4281effea56f"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_favorite selections cell_multipleSelect s-cell">
        <div title="">
            <div id="favorite_509bb871-b964-48ab-94fe-fb098cebc04d"
                 class="passbolt_controller_component_favorite_controller passbolt_view_component_favorite js_component ready">
                <a href="#" class="no-text">
                    <i class="icon fav"></i>
                    <span>fav</span>
                </a>
            </div>
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="bank password">
            bank password
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="passbolt">
            passbolt
        </div>
    </td>

    <td class="js_grid_column_secret m-cell password">
        <div title="">
            <div class="secret-copy"><a href="#copy_secret"><span>copy password to clipboard</span></a><pre>-----BEGIN PGP MESSAGE-----
Version: GnuPG v1.4.12 (GNU/Linux)


=DzJr
-----END PGP MESSAGE-----
</pre>
            </div>
        </div>
    </td>

    <td class="js_grid_column_uri l-cell">
        <div title="https://bank.com">
            https://bank.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="2 years ago">
            2 years ago
        </div>
    </td>

    <td class="js_grid_column_owner m-cell">
        <div title="ismail@passbolt.com">
            ismail@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_copyLogin s-cell">
        <div title="">

        </div>
    </td>

    <td class="js_grid_column_copySecret s-cell">
        <div title="">

        </div>
    </td>

</tr>
</tbody>
</table>
</div>
</div>
</div>
<div class="panel aside js_wsp_pwd_sidebar_second passbolt_controller_component_resource_details_controller"
     style="display:none" id="63ae8e20-ed2c-1801-d7ae-475e99bafd8e">
</div>
</div>
</div>
<div id="js_passbolt_peopleWorkspace_controller"
     class="passbolt_controller_people_workspace_controller mad_view_view tab-content ready selected">
<div class="js_users_workspace">
<div class="panel left">
    <div class="navigation first shortcuts">
        <ul id="js_wsp_users_group_shortcuts"
            class="clearfix passbolt_controller_component_user_shortcuts_controller mad_view_component_tree menu ready">
            <li id="1ed44d81-9be9-c970-5904-8f232b87c563" class="ready">
                <div class="row">
                    <div class="main-cell-wrapper">
                        <div class="main-cell">
                            <a href="#"><span>All users</span></a>
                        </div>
                    </div>
                </div>


            </li>
            <li id="ee838497-8480-137a-dee0-099bbfebbb9e" class="ready">
                <div class="row">
                    <div class="main-cell-wrapper">
                        <div class="main-cell">
                            <a href="#"><span>Recently modified</span></a>
                        </div>
                    </div>
                </div>


            </li>
        </ul>
    </div>
    <div class="navigation last tree groups">
        <ul id="js_wsp_users_group_chooser">
        </ul>
    </div>
</div>
<div class="panel middle">
<div id="js_wsp_ppl_breadcrumb"
     class="breadcrumbs passbolt_controller_component_people_breadcrumb_controller mad_view_view js_component ready">
    <ul class="mad_controller_component_menu_controller mad_view_component_tree menu ready"
        id="d71ec42c-8e16-476c-5f7f-d63876555175">
        <li id="fe60c6b2-5277-2fcc-6858-f2f6c668f387" class="ready">
            <div class="main-cell">
                <a href="#"><span>All users</span></a>
            </div>
        </li>
    </ul>
</div>
<div id="js_wsp_ppl_browser"
     class="tableview passbolt_controller_component_user_browser_controller passbolt_view_component_user_browser selection">
<div class="tableview-header">
    <table>
        <thead>
        <tr>

            <th class="js_grid_column js_grid_column_multipleSelect cell_multipleSelect selections cell_multipleSelect s-cell">
                <div class="input checkbox"><label for="checkbox-select-all">select
                    all</label></div>
            </th>

            <th class="js_grid_column js_grid_column_avatar s-cell cell_avatar">

            </th>

            <th class="js_grid_column js_grid_column_name m-cell">
                User
            </th>

            <th class="js_grid_column js_grid_column_username m-cell">
                Username
            </th>

            <th class="js_grid_column js_grid_column_modified m-cell">
                Modified
            </th>

        </tr>
        </thead>
    </table>
</div>
<div class="tableview-content scroll">
<table>
<tbody>
<tr id="dada6042-c5cd-11e1-a0c5-080027796c51">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_dada6042-c5cd-11e1-a0c5-080027796c51"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="dada6042-c5cd-11e1-a0c5-080027796c51"
                           id="checkboxc49705e1-42c4-49d6-5052-038ce441dae7">
                    <label for="checkboxc49705e1-42c4-49d6-5052-038ce441dae7"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png" width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Root Abaga">
            Root Abaga
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="root@passbolt.com">
            root@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="50cdea9c-7e80-4eb6-b4cc-2f4fd7a10fce" class="selected">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_50cdea9c-7e80-4eb6-b4cc-2f4fd7a10fce"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="50cdea9c-7e80-4eb6-b4cc-2f4fd7a10fce"
                           id="checkbox780d5fcf-e5f9-0159-2983-a8a9ba099444"
                           checked="checked">
                    <label for="checkbox780d5fcf-e5f9-0159-2983-a8a9ba099444"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png"
                 width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Cedric Alfonsi">
            Cedric Alfonsi
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="cedric@passbolt.com">
            cedric@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="50cdea9c-4380-4eb6-b4cc-2f4fd7a10fce">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_50cdea9c-4380-4eb6-b4cc-2f4fd7a10fce"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="50cdea9c-4380-4eb6-b4cc-2f4fd7a10fce"
                           id="checkbox8066389e-ab37-ab35-3de0-055af9b2f7f4">
                    <label for="checkbox8066389e-ab37-ab35-3de0-055af9b2f7f4"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png" width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Jean René Bergamotte">
            Jean René Bergamotte
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="jean-rene@test.com">
            jean-rene@test.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="50cdea9c-a34c-406f-a9f1-2f4fd7a10fce">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_50cdea9c-a34c-406f-a9f1-2f4fd7a10fce"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="50cdea9c-a34c-406f-a9f1-2f4fd7a10fce"
                           id="checkbox8789f715-0e4f-03ec-3b58-90e4aa4bd616">
                    <label for="checkbox8789f715-0e4f-03ec-3b58-90e4aa4bd616"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png"
                 width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Remy Bertot">
            Remy Bertot
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="remy@passbolt.com">
            remy@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="533d37a0-aa11-4945-9b11-1663a0a895dc">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_533d37a0-aa11-4945-9b11-1663a0a895dc"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="533d37a0-aa11-4945-9b11-1663a0a895dc"
                           id="checkbox304d09fc-3f92-3f7b-fa57-6cd819af0124">
                    <label for="checkbox304d09fc-3f92-3f7b-fa57-6cd819af0124"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png" width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Userone Company A">
            Userone Company A
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="a-usr1@companya.com">
            a-usr1@companya.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="50cdea9c-fa10-47af-aaa8-2f4fd7a10fce" class="">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_50cdea9c-fa10-47af-aaa8-2f4fd7a10fce"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="50cdea9c-fa10-47af-aaa8-2f4fd7a10fce"
                           id="checkbox5c89392e-3f39-d039-1fe2-b865e22b2334">
                    <label for="checkbox5c89392e-3f39-d039-1fe2-b865e22b2334"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png"
                 width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Myriam Djerouni">
            Myriam Djerouni
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="myriam@passbolt.com">
            myriam@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="50cdea9c-f214-4549-9807-2f4fd7a10fce">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_50cdea9c-f214-4549-9807-2f4fd7a10fce"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="50cdea9c-f214-4549-9807-2f4fd7a10fce"
                           id="checkbox969161fd-7222-7191-91b4-f2966889a39b">
                    <label for="checkbox969161fd-7222-7191-91b4-f2966889a39b"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png" width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Aurelie Gherards">
            Aurelie Gherards
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="aurelie@passbolt.com">
            aurelie@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="50cdea9c-af80-4e5e-86d0-2f4fd7a10fce">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_50cdea9c-af80-4e5e-86d0-2f4fd7a10fce"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="50cdea9c-af80-4e5e-86d0-2f4fd7a10fce"
                           id="checkboxcb5cb68a-633e-cadc-6dd5-a75542bd893f">
                    <label for="checkboxcb5cb68a-633e-cadc-6dd5-a75542bd893f"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png"
                 width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Ismail Guennouni">
            Ismail Guennouni
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="ismail@passbolt.com">
            ismail@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="533d37a0-bc80-4945-9b11-1663c0a895dc">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_533d37a0-bc80-4945-9b11-1663c0a895dc"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="533d37a0-bc80-4945-9b11-1663c0a895dc"
                           id="checkbox456e0f06-2bae-5ce6-dad8-325e9499bd99">
                    <label for="checkbox456e0f06-2bae-5ce6-dad8-325e9499bd99"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png" width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Admin Istrator">
            Admin Istrator
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="admin@passbolt.com">
            admin@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="50cdab9c-4380-4eb6-b4cc-2f4fd7a10fce">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_50cdab9c-4380-4eb6-b4cc-2f4fd7a10fce"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="50cdab9c-4380-4eb6-b4cc-2f4fd7a10fce"
                           id="checkbox769fa67b-4b7c-8515-01ca-2569c99c5352">
                    <label for="checkbox769fa67b-4b7c-8515-01ca-2569c99c5352"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png" width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="User Lambda">
            User Lambda
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="user@passbolt.com">
            user@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="533d346d-d378-4acc-affd-1663c0a895dc">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_533d346d-d378-4acc-affd-1663c0a895dc"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="533d346d-d378-4acc-affd-1663c0a895dc"
                           id="checkbox70b375d6-44f5-bfb1-d706-3d507388e54e">
                    <label for="checkbox70b375d6-44f5-bfb1-d706-3d507388e54e"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png" width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Frank Leboeuf">
            Frank Leboeuf
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="frank@passbolt.com">
            frank@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="533d32c0-1f30-438c-8f26-1768c0a895dc">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_533d32c0-1f30-438c-8f26-1768c0a895dc"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="533d32c0-1f30-438c-8f26-1768c0a895dc"
                           id="checkboxfb4c01bd-b259-f8b8-6020-522db7c8be8b">
                    <label for="checkboxfb4c01bd-b259-f8b8-6020-522db7c8be8b"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png" width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Great Manager">
            Great Manager
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="manager.nogroup@passbolt.com">
            manager.nogroup@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="50cdea9c-aa88-46cb-a09b-2f4fd7a10fce">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_50cdea9c-aa88-46cb-a09b-2f4fd7a10fce"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="50cdea9c-aa88-46cb-a09b-2f4fd7a10fce"
                           id="checkbox1449e1ca-4660-bb5e-49c0-640c5972b682">
                    <label for="checkbox1449e1ca-4660-bb5e-49c0-640c5972b682"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png"
                 width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Kevin Muller">
            Kevin Muller
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="kevin@passbolt.com">
            kevin@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="533d3564-03e8-4963-94a7-178cc0a895dc">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_533d3564-03e8-4963-94a7-178cc0a895dc"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="533d3564-03e8-4963-94a7-178cc0a895dc"
                           id="checkbox1077cf58-2c57-106d-e909-9c623e383716">
                    <label for="checkbox1077cf58-2c57-106d-e909-9c623e383716"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png" width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Guest Star">
            Guest Star
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="guest@passbolt.com">
            guest@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="bbd56042-c5cd-11e1-a0c5-080027796c4e">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_bbd56042-c5cd-11e1-a0c5-080027796c4e"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="bbd56042-c5cd-11e1-a0c5-080027796c4e"
                           id="checkboxc092b9e3-2684-d7dc-f320-7322077f123f">
                    <label for="checkboxc092b9e3-2684-d7dc-f320-7322077f123f"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png" width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="User b Test">
            User b Test
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="test@passbolt.com">
            test@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="eeee6042-c5cd-11e1-a0c5-080027796c51">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_eeee6042-c5cd-11e1-a0c5-080027796c51"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="eeee6042-c5cd-11e1-a0c5-080027796c51"
                           id="checkbox132289a1-2e71-006b-7cc1-610bd70cc9da">
                    <label for="checkbox132289a1-2e71-006b-7cc1-610bd70cc9da"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png" width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="User Test">
            User Test
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="utest@passbolt.com">
            utest@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="dada6042-c5cd-11e1-a0c5-080027796c4c">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_dada6042-c5cd-11e1-a0c5-080027796c4c"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="dada6042-c5cd-11e1-a0c5-080027796c4c"
                           id="checkbox92d58d0e-9869-59ec-5556-47b14b88b829">
                    <label for="checkbox92d58d0e-9869-59ec-5556-47b14b88b829"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png"
                 width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Darth Vader">
            Darth Vader
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="darth.vader@passbolt.com">
            darth.vader@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
<tr id="bbd56042-c5cd-11e1-a0c5-080027796c4c">

    <td class="js_grid_column_multipleSelect selections cell_multipleSelect s-cell">
        <div title="">
            <div id="multiple_select_checkbox_bbd56042-c5cd-11e1-a0c5-080027796c4c"
                 class="mad_form_element_checkbox_controller mad_view_form_element_checkbox_view js_checkbox_multiple_select ready">
                <div class="input checkbox">
                    <input type="checkbox"
                           value="bbd56042-c5cd-11e1-a0c5-080027796c4c"
                           id="checkbox6c656410-7cb2-5c03-7d98-a0a0e74d9d46">
                    <label for="checkbox6c656410-7cb2-5c03-7d98-a0a0e74d9d46"></label>
                </div>
            </div>
        </div>
    </td>

    <td class="js_grid_column_avatar s-cell cell_avatar">
        <div title="">
            <img src="img/avatar/user.png" width="25" height="25">
        </div>
    </td>

    <td class="js_grid_column_name m-cell">
        <div title="Anon Ymous">
            Anon Ymous
        </div>
    </td>

    <td class="js_grid_column_username m-cell">
        <div title="anonymous@passbolt.com">
            anonymous@passbolt.com
        </div>
    </td>

    <td class="js_grid_column_modified m-cell">
        <div title="6 days ago">
            6 days ago
        </div>
    </td>

</tr>
</tbody>
</table>
</div>
</div>
</div>
<div class="panel aside js_wsp_users_sidebar_second passbolt_controller_component_user_details_controller passbolt_view_component_user_details js_component ready"
     style="" id="06b0333e-fa7b-50f2-38e5-26ab6f21d0d1">
    <div class="user">
        <div class="header">
            <img src="img/avatar/user.png">
            <h3>
                <span class="name">Cedric Alfonsi</span>
                <span class="username">cedric@passbolt.com</span>
            </h3>
        </div>
        <a href="#" class="dialog-close"><i
                class="icon close no-text"></i><span>close</span></a>

        <div class="detailed-information">
            <h4>Information</h4>
            <ul>
                <li class="username">
                    <span class="label">Password</span>
                    <span class="value">**********</span>
                </li>
                <li class="password">
                    <span class="label">Role</span>
                    <span class="value">User</span>
                </li>
                <li class="modified">
                    <span class="label">Modified</span>
                    <span class="value">6 days ago</span>
                </li>
            </ul>
        </div>
        <div class="key-information">
            <h4>Public Key</h4>
            <ul>
                <li class="keyid">
                    <span class="label">Key id</span>
                    <span class="value">36E2C9F9</span>
                </li>
                <li class="type">
                    <span class="label">Type</span>
                    <span class="value">RSA</span>
                </li>
                <li class="created">
                    <span class="label">Created</span>
                    <span class="value">2015-03-20 14:33:40</span>
                </li>
                <li class="expires">
                    <span class="label">Expires</span>
                    <span class="value"></span>
                </li>
                <li class="key">
                    <span class="label">Public key</span>
				<span class="value">
					<a href="#" class="button copy-public-key">
                        <span>copy</span>
                    </a>
				</span>
                </li>
                <li>
                    <textarea class="code">-----BEGIN PGP PUBLIC KEY BLOCK-----
                        Version: GnuPG/MacGPG2 v2.0.22 (Darwin)
                        Comment: GPGTools - https://gpgtools.org

                        
                        EqYVvACgtOksdPIONyG8tJN1rT0yDasL07qA4ZqG4zOhrWJXppblKnSc
                        =iiFb
                        -----END PGP PUBLIC KEY BLOCK-----
                    </textarea>
                </li>
            </ul>
        </div>
    </div>
</div>
</div>
</div>
<div id="js_passbolt_preferenceWorkspace_controller"
     class="passbolt_controller_preference_workspace_controller"></div>
</div>
</div>
<div class="footer">
    <span class="copyright"></span> •
    <span class="version">2.13.3</span>
</div>
</div>
</div>
<!-- footer -->
<footer>
    <div class="footer">
        <span class="copyright">2013 © Passbolt.com</span> •
        <span class="version">v.2.13.3</span>
    </div>
</footer>
<!--<script type="text/javascript" src="/js/lib/can/lib/jquery.1.8.3.js"></script>-->
<!--<script type="text/javascript"-->
        <!--src="/js/lib/xregexp/xregexp-all-min.js"></script>-->
<!--<script type="text/javascript"-->
        <!--src="/js/lib/jquery/jquery-ui-1.10.3.custom.js"></script>-->
<!--<script type="text/javascript" src="/js/lib/moment/moment.min.js"></script>-->
<!--<script type="text/javascript"-->
        <!--src="/js/steal/steal.js?app/passbolt.js"></script>-->

</body>
</html>