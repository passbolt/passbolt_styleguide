...
export class PasswordSettingsContextProvider extends React.Component {
  ...
  constructor(props) {
    ...
    if (this.props.context?.getApiClientOptions) {
      this.passwordPolicyService = new PasswordPolicyService(this.props.context.getApiClientOptions());
    }
  }
   ...
   */
  async findPolicies() {
    if (!this.props.context.siteSettings.canIUse('passwordPolicies') || this.getPolicies() !== null) {
      return;
    }

    this.setProcessing(true);
    const result = this.passwordPolicyService
      ? await this.passwordPolicyService.find()
      : await this.props.context.port.request("passbolt.password-policies.get");
