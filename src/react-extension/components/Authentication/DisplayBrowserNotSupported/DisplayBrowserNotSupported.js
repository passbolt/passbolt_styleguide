  ...
  render() {
    return (
      <div className="browser-not-supported">
        ...
        <p>
          <Trans>Please download one of these browsers to get started with passbolt:</Trans>
        </p>
        <div className="browser-button-list">
          {this.compatibleBrowserList.map((browserInfo, key) => (
            <button
              key={key}
              className={`browser${browserInfo.name === this.state.selectedBrowser.name ? " focused" : ""}`}
              target="_blank"
        ...
        <div className="form-actions">
          <a
            ...
            target="_blank"
          >
            <Trans>Download {{ browserName: this.state.selectedBrowser.name }}</Trans>
          </a>
        </div>
      </div>
    );
