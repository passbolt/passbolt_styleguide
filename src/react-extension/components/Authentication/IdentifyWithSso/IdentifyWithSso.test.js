    ...
    it.each([
      {
        ...
        recoverUrl: `https://malicious.passbolt.test/login?back=${window.location.origin}/setup/recover/${uuid()}/${uuid()}`,
      },
      {
        location: "fragment",
        recoverUrl: `https://malicious.passbolt.test/login#back=${window.location.origin}/setup/recover/${uuid()}/${uuid()}`,
      ...
      "As AN I should not be redirected if a foreign origin smuggles the current origin in its $location",
      async ({ recoverUrl }) => {
        expect.assertions(2);
        const popupUrl = "https://third-party.auth.com";
        const expectedToken = uuid();
