...
class EditSubscriptionKey extends Component {
  ...
  async handleSaveSuccess() {
    await this.props.actionFeedbackContext.displaySuccess(
      this.translate("The subscription key has been updated successfully."),
    );
      ...
      >
        <form onSubmit={this.handleFormSubmit} noValidate>
          <div className="form-content">
            <div
              className={`input textarea required ${this.state.keyError ? "error" : ""} ${this.hasAllInputDisabled() ? "disabled" : ""}`}
            >
              <label htmlFor="edit-tag-form-name">
                <Trans>Subscription key</Trans>
              </label>
              <textarea
                id="edit-subscription-form-key"
                name="key"
                ...
                className="required full_report"
                required="required"
                autoComplete="off"
                autoFocus={true}
              />
...
export default withAppContext(
  withAdminSubscription(
    withAdministrationWorkspace(withActionFeedback(withDialog(withTranslation("common")(EditSubscriptionKey)))),
  ),
);
