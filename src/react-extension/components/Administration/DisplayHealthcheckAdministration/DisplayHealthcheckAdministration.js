...
class DisplayHealthcheckAdministration extends Component {
  ...
  async componentDidMount() {
    this.props.administrationWorkspaceContext.setDisplayAdministrationWorkspaceAction(
      DisplayAdministrationHealthcheckActions,
    );
  ...
  render() {
          ...
          <span className="healthcheck-fail">
            ...
            <Tooltip
              message={this.props.t(
                "Run the install script to set the dafault content such as roles and permission types",
              )}
            >
              <InfoSVG className="baseline svg-icon" />
            </Tooltip>
      ...
      if (healthcheckData.environment.tmpWritable === true) {
        ...
        return (
          <span className="healthcheck-fail">
            ...
            <Tooltip
              message={this.props.t(
                "Ensure the temporary directory and its content are writable by the webserver user.",
              )}
            >
              <InfoSVG className="baseline svg-icon" />
            </Tooltip>
            ...
            <Tooltip
              message={
                <span>
                  <Trans>
                    See{" "}
                    <a href="https://secure.php.net/manual/en/book.image.php" target="_blank" rel="noopener noreferrer">
                      this guide
                    </a>
                  </Trans>
                </span>
            ...
            <Tooltip
              message={
                <span>
                  <Trans>
                    See{" "}
                    <a href="https://secure.php.net/manual/en/book.intl.php" target="_blank" rel="noopener noreferrer">
                      this guide
                    </a>
                  </Trans>
                </span>
              ...
              message={
                <span>
                  <Trans>
                    See{" "}
                    <a
                      href="https://secure.php.net/manual/en/book.mbstring.php"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      this guide
          ...
          <span className="healthcheck-fail">
            ...
            <Tooltip
              message={
                <span>
                  <Trans>
                    Install php-gnupg, see{" "}
                    <a href="http://php.net/manual/en/gnupg.installation.php" target="_blank" rel="noopener noreferrer">
                      this guide
                    </a>
                  </Trans>
                </span>
    ...
    const isPublicKeyDefined = () => {
      const configurationFilePath = healthcheckData.application.configPath.toString();
      if (
        healthcheckData.gpg.gpgKeyPublic === true &&
        healthcheckData.gpg.gpgKeyPublicReadable === true &&
        healthcheckData.gpg.gpgKeyPublicBlock
      ) {
        return (
          <span className="healthcheck-success">
            <HealthcheckSuccessSVG />
    ...
    const isPrivateKeyDefined = () => {
      const configurationFilePath = healthcheckData.application.configPath.toString();
      if (
        healthcheckData.gpg.gpgKeyPrivate === true &&
        healthcheckData.gpg.gpgKeyPrivateReadable === true &&
        healthcheckData.gpg.gpgKeyPrivateBlock
      ) {
        return (
          <span className="healthcheck-success">
            <HealthcheckSuccessSVG />
    ...
    const isServerFingerprintMatchingConfigFile = () => {
      const configurationFilePath = healthcheckData.application.configPath.toString();
      if (
        healthcheckData.gpg.gpgKeyPrivateFingerprint === true &&
        healthcheckData.gpg.gpgKeyPublicFingerprint === true
      ) {
        return (
          <span className="healthcheck-success">
            <HealthcheckSuccessSVG />
    ...
    const isUsingLatestVersion = () => {
      if (healthcheckData.application.latestVersion === true && healthcheckData.application.info.remoteVersion) {
        ...
        healthcheckData.application.latestVersion === false &&
        healthcheckData.application.info.remoteVersion
      ) {
        ...
        return (
          <span className="healthcheck-fail">
            <HealthcheckErrorSVG />
            <Trans>
              The installation is not up to date. Currently using {{ currentVersion }} and it should be{" "}
        ...
        healthcheckData.application.latestVersion === null &&
        healthcheckData.application.info.remoteVersion === "undefined"
      ) {
        return (
          <span className="healthcheck-fail">
            <HealthcheckErrorSVG />
    ...
    const isRegistrationClosed = () => {
      if (healthcheckData.application.registrationClosed.selfRegistrationProvider === null) {
        ...
        const selfRegistrationProvider =
          healthcheckData.application.registrationClosed.selfRegistrationProvider.toString();
        return (
          <span className="healthcheck-info">
            <HealthcheckInfoSVG />
        ...
        if (healthcheckData.smtpSettings.isInDb === true) {
          ...
          return (
            <span className="healthcheck-fail">
              ...
              <Tooltip
                message={this.props.t(
                  "It is recommended to set the SMTP Settings in the database through the administration section.",
                )}
              >
                <InfoSVG className="baseline svg-icon" />
              </Tooltip>
      ...
      if (healthcheckData.directorySync.endpointsDisabled === true) {
        ...
        return (
          <span className="healthcheck-warning">
            ...
            <Tooltip
              message={this.props.t(
                "It is recommended to disable endpoints for updating the users directory configurations.",
              )}
            >
              <InfoSVG className="baseline svg-icon" />
            </Tooltip>
      ...
      if (healthcheckData.metadata.canDecryptMetadataPrivateKey === true) {
        ...
        return (
          <span className="healthcheck-fail">
            ...
            <Tooltip
              message={this.props.t(
                "For more information, please run the health check from the command line on the server.",
              )}
            >
              <InfoSVG className="baseline svg-icon" />
            </Tooltip>
      ...
      if (healthcheckData.metadata.isServerHasAccessToMetadataKey === true) {
        ...
        return (
          <span className="healthcheck-fail">
            ...
            <Tooltip
              message={this.props.t(
                "When zero-knowledge mode is off, the server must have access to the metadata key. Without having access, the server won\'t be able to share the metadata private key with the users.",
              )}
            >
              <InfoSVG className="baseline svg-icon" />
            </Tooltip>
          ...
          {createSafePortal(
            <>
              <div className="sidebar-help-section">
                <h3>
                  <Trans>What is this page?</Trans>
                </h3>
                <p>
                  <Trans>
                    This page is available to help administrators diagnose if something is wrong with a passbolt
                    installation and help keeping it secure.
                  </Trans>
                </p>
                <p>
                  <Trans>
                    The color is really important here so it&apos;s easier for you to spot what&apos;s not running as
                    expected
                  </Trans>
                </p>
                <div className="healthcheck-color-legends">
                  ...
                  <div className="healthcheck-warning">
                    <TriangleAlertSVG />{" "}
                    <Trans>
                      Something inside your configuration is not what we recommend, but you can skip it if it has been
                      done on purpose.
                    </Trans>
                  </div>
                  <div className="healthcheck-fail">
                    <HealthcheckErrorSVG />{" "}
                    <Trans>There is an error with the current configuration, you might want to resolve it.</Trans>
                  </div>
              ...
              <div className="sidebar-help-section">
                <h3>
                  <Trans>Something wrong?</Trans>
                </h3>
                <p>
                  <Trans>
                    Hang in there! Depending your installation, you might need to check the documentation in order to
                    run the healthcheck from the CLI
                  </Trans>
                </p>
                <a
                  className="button"
                  href="https://www.passbolt.com/docs/admin/server-maintenance/passbolt-api-status/"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <FileTextSVG />
...
export default withAppContext(
  withAdministrationWorkspace(
    withAdministrationHealthcheck(withTranslation("common")(DisplayHealthcheckAdministration)),
  ),
);
