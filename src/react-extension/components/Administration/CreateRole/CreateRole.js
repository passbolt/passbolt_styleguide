...
class CreateRole extends Component {
   ...
   * @param {object} resourceFormEntityDto The form role entity dto settings dto store in state, not used but required to ensure the memoized
   *   function is only triggered when the form is updated.
   * @return {EntityValidationError}
   */
  // eslint-disable-next-line no-unused-vars
  verifyDataHealth = memoize(roleDto => this.roleEntity.verifyHealth());
        ...
        <form className="role-create-form" onSubmit={this.handleFormSubmit} noValidate>
          <div className="form-content">
            <div className={`input text required ${errors?.hasError("name") || hasNameReservedError ? "error" : ""} ${this.hasAllInputDisabled() ? 'disabled' : ''}`}>
              <label htmlFor="role-name-input"><Trans>Name</Trans>{this.state.nameWarning &&
