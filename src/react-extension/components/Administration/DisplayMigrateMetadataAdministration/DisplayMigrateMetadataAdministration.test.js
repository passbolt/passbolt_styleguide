...
import DisplayMigrateMetadataAdministrationPage from "./DisplayMigrateMetadataAdministration.test.page";
...
describe("DisplayMigrateMetadataAdministration as per the specifications", () => {
  it("As a signed-in administrator I can see migration metadata form", async () => {
    ...
    let page;
    await act(async () => (page = new DisplayMigrateMetadataAdministrationPage(props)));
    ...
    expect(page.formWarningBanner.textContent).toBe(
      "Warning: If you have integrations, you will have to make sure they are updated before triggering the migration.",
    );
  });
  ...
  it("As a signed-in administrator I can see migration metadata form disabled", async () => {
    ...
    expect(page.formErrorBanner.textContent).toBe(
      "You lack access to the shared metadata key. Please ask another administrator to share it with you.",
    );
  });
