...
describe("As AD I should show the subscription announcement", () => {
  ...
  describe("As LU I can see the subscription announcement", () => {
    ...
    it("As LU I should show the subscription announcement if the subscription is going to expire", async () => {
      ...
      jest
        .spyOn(props.context, "onGetSubscriptionKeyRequested")
        .mockImplementationOnce(goingToExpireSubscriptionGetRequestMock);
      new HandleSubscriptionAnnouncementPage(props);
      await waitFor(() => {});
      expect(props.announcementContext.show).toHaveBeenCalledWith(DisplayGoingToExpireSubscriptionAnnouncement, {
        expiry: mockSubscriptionGoingToExpired.expiry,
      });
    });

    it("As LU I should see the subscription announcement if the subscription is expired", async () => {
      const expiredSubscriptionGetRequestMock = jest.fn(() => {
        throw new PassboltSubscriptionError();
      });
      jest
        .spyOn(props.context, "onGetSubscriptionKeyRequested")
        .mockImplementationOnce(expiredSubscriptionGetRequestMock);
      new HandleSubscriptionAnnouncementPage(props);
      await waitFor(() => {});
