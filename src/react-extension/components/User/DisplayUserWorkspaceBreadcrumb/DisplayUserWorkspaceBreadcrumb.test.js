...
import DisplayUserWorkspaceBreadcrumbPage from "./DisplayUserWorkspaceBreadcrumb.test.page";
import {
  defaultAppContext, propsWithEmptyGroupNameFilter, propsWithEmptyTextFilter,
  propsWithFilter,
  propsWithGroupFilter,
  propsWithTextFilter
} from "./DisplayUserWorkspaceBreadcrumb.test.data";
...
describe("Display User Workspace Breadcrumb", () => {
  ...
  it('As LU, I should see "All Users" if the filter is set to All Users', async() => {
    page = new DisplayUserWorkspaceBreadcrumbPage(context, propsWithFilter(UserWorkspaceFilterTypes.ALL));
    await waitFor(() => {});
  ...
  it('As LU, I should see "Recently Modified" if the filter is set to Recently Modified', async() => {
    page = new DisplayUserWorkspaceBreadcrumbPage(context, propsWithFilter(UserWorkspaceFilterTypes.RECENTLY_MODIFIED));
    await waitFor(() => {});
    expect(page.breadcrumbLabels(1)).toBe('All users');
  ...
  it('As LU, I should see All Users if the filter is set to an non-empty Text', async() => {
    page = new DisplayUserWorkspaceBreadcrumbPage(context, propsWithEmptyTextFilter());
    await waitFor(() => {});
    expect(page.breadcrumbLabels(1)).toBe('All users');
  ...
  it('As LU, I should see the group name if the filter is set to Group', async() => {
    page = new DisplayUserWorkspaceBreadcrumbPage(context, propsWithGroupFilter());
    await waitFor(() => {});
    expect(page.breadcrumbLabels(1)).toBe('All users');
