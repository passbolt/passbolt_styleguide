  ...
  async handleConfirmShare(event) {
    ...
    try {
      ...
      this.props.actionFeedbackContext.displaySuccess(
        this.props.t("Metadata keys have been shared with ({{username}})", { username: this.props.user.username }),
      );
    } catch (error) {
      console.error(error);
      if (error.message) {
  ...
  render() {
    ...
    return (
      <DialogWrapper
        ...
        title={this.props.t("Share missing metadata keys?")}
        ...
        <form onSubmit={this.handleConfirmShare} noValidate>
          <div className="form-content">
            {loggedInUserMissingMetadataKeys.length === 0 && (
              <p>
                {this.props.t("Share missing metadata keys with {{name}} ({{username}})", {
            ...
            {this.numberOfKeysCanBeShared === 0 && (
              <p>
                {this.props.t(
                  "You can't share metadata keys with {{name}} ({{username}}) because you don't have access to them. Ask another administrator for help.",
                  { name: this.userFullName, username: this.props.user.username },
                )}
              </p>
            )}
            {this.numberOfKeysCanBeShared > 0 && this.numberOfKeysCanBeShared < userMissingMetadataKeys.length && (
              <p>
                {this.props.t(
                  "Share your available metadata keys with {{name}} ({{username}})? You're missing some keys yourself. Ask another administrator to share the remaining keys.",
                  { name: this.userFullName, username: this.props.user.username },
                )}
              </p>
            )}
...
export default withAppContext(
  withActionFeedback(withUserWorkspace(withTranslation("common")(ConfirmShareMissingMetadataKeys))),
);
