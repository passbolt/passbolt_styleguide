...
} from "./FolderSidebar.test.data";
import FolderSidebarPage from "./FolderSidebar.test.page";
import {ActionFeedbackContext} from "../../../contexts/ActionFeedbackContext";

jest.mock("./FolderSidebarInformationSection", () => () => <></>);
jest.mock("./FolderSidebarActivitySection", () => () => <></>);
jest.mock("./FolderSidebarPermissionsSection", () => () => <></>);
...
describe("See Resource Sidebar", () => {
  ...
  describe('As LU I can see a folder', () => {
    ...
    beforeEach(() => {
      page = new FolderSidebarPage(context, props);
    });
    ...
    it('As LU I should be able to identify the name and the permalink', async() => {
      ...
      expect(context.port.request).toHaveBeenCalledWith("passbolt.clipboard.copy", `${context.userSettings.getTrustedDomain()}/app/folders/view/${props.resourceWorkspaceContext.details.folder.id}`);
