...
import { MetadataTypesSettingsLocalStorageContext } from "../../../../shared/context/MetadataTypesSettingsLocalStorageContext/MetadataTypesSettingsLocalStorageContext";
            ...
            <DialogContextProvider>
              <ActionFeedbackContext.Provider value={args.actionFeedbackContext}>
                <MetadataTypesSettingsLocalStorageContext.Provider
                  value={{ get: () => args.metadataTypeSettings, metadataTypeSettings: args.metadataTypeSettings }}
                >
                  <ResourceTypesLocalStorageContext.Provider
                    value={{ get: () => args.resourceTypes, resourceTypes: args.resourceTypes }}
                  >
...
mockedDefaultPort.addRequestListener("passbolt.secret.find-by-resource-id", () => ({
  password: "secret-decrypted",
  description: "description",
}));

const mockedPortPasswordDescriptionTotp = new MockPort();
mockedPortPasswordDescriptionTotp.addRequestListener("passbolt.secret.find-by-resource-id", () => ({
  password: "secret-decrypted",
  description: "description",
  totp: defaultTotpDto(),
}));
...
mockedPasswordStringPort.addRequestListener("passbolt.secret.find-by-resource-id", () => ({
  password: "secret-decrypted",
}));

const mockedPortCustomFields = new MockPort();
const customFieldsProps = defaultCustomFieldsProps();
mockedPortCustomFields.addRequestListener("passbolt.secret.find-by-resource-id", () => ({
  custom_fields: [
    { id: customFieldsProps.resource.metadata.custom_fields[0].id, type: "text", secret_value: "secret-0" },
  ],
}));
...
const defaultV5DefaultTotpProps = defaultProps({
  resource: defaultResourceDto({ resource_type_id: TEST_RESOURCE_TYPE_V5_DEFAULT_TOTP }),
});
...
const defaultV5StringProps = defaultProps({
  resource: defaultResourceDto({ resource_type_id: TEST_RESOURCE_TYPE_V5_PASSWORD_STRING }),
});
...
const defaultLegacyProps = defaultProps({
...
const defaultV4DefaultTotpProps = defaultProps({
  resource: defaultResourceDto({ resource_type_id: TEST_RESOURCE_TYPE_PASSWORD_DESCRIPTION_TOTP }),
});
...
const legacyTotpProps = defaultTotpProps({
  resource: defaultResourceDto({ resource_type_id: TEST_RESOURCE_TYPE_TOTP }),
});
...
const legacyStringProps = defaultProps({
