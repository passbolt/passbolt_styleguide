 ...
 */
class DisplayUserIntroductionPassphrase extends React.Component {
  /**
   * Default constructor
  ...
  async validateUnderstandPassphrase() {
    let understandPassphraseError = false;
    if (!this.state.understandPassphrase) {
      understandPassphraseError = true;
    }
        ...
        <div className="row">
          <div className="col6">
            <form onSubmit={this.handleSubmit}>
              ...
              <div>
                <img src={`${this.context.userSettings.getTrustedDomain()}/img/illustrations/passphrase_intro.png`} />
              </div>
              <p>The passphrase is stored on your device and never send server side.<br/>
                Changing your passphrase will only change it locally.
                If you have multiple browser configured, the passphrase will need to be changed in all places individually.
              </p>
...
DisplayUserIntroductionPassphrase.contextType = AppContext;
DisplayUserIntroductionPassphrase.propTypes = {
  userSettingsContext: PropTypes.object // The user settings context
};

export default withUserSettings(DisplayUserIntroductionPassphrase);
