...
class HealthcheckEntity extends Entity {
  ...
  constructor(dto) {
    ...
    // Associations
    if (this._props.ssl) {
      this._ssl = new SslEntity(this._props.ssl);
    }
    delete this._props.ssl;

    if (this._props.database) {
      this._database = new DatabaseEntity(this._props.database);
    }
    delete this._props.database;

    if (this._props.application) {
      this._application = new ApplicationEntity(this._props.application);
    }
    delete this._props.application;

    if (this._props.gpg) {
      this._gpg = new GpgEntity(this._props.gpg);
    }
    delete this._props.gpg;

    if (this._props.environment) {
      this._environment = new EnvironmentEntity(this._props.environment);
    }
    delete this._props.environment;

    if (this._props.configFile) {
      this._configFile = new ConfigFileEntity(this._props.configFile);
    }
    delete this._props.configFile;

    if (this._props.core) {
      this._core = new CoreEntity(this._props.core);
    }
    delete this._props.core;

    if (this._props.smtpSettings) {
      this._smtpSettings = new SmtpSettingsEntity(this._props.smtpSettings);
    }
