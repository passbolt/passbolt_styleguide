...
describe("Resource Form entity", () => {
  ...
  describe("::removeEmptySecret", () => {
    it("remove totp secret and add require password secret from v4 password, description and totp resource", () => {
      ...
      const resourceDto = defaultResourceFormDto({
        resource_type_id: TEST_RESOURCE_TYPE_PASSWORD_DESCRIPTION_TOTP,
        secret: { totp: defaultTotpDto({ secret_key: "" }), description: "" },
      });
      const resourceTypeDtos = resourceTypesCollectionDto();
      const resourceTypesCollection = new ResourceTypesCollection(resourceTypeDtos);
    ...
    it("remove totp secret should set a standalone note from v5 default totp resource", () => {
      ...
      const resourceDto = defaultResourceFormDto({
        resource_type_id: TEST_RESOURCE_TYPE_V5_DEFAULT_TOTP,
        secret: { object_type: SECRET_DATA_OBJECT_TYPE, totp: defaultTotpDto({ secret_key: "" }), description: "" },
      });
      const resourceTypeDtos = resourceTypesCollectionDto();
      const resourceTypesCollection = new ResourceTypesCollection(resourceTypeDtos);
    ...
    it("remove custom fields secret and add require password secret from v5 default totp resource", () => {
      ...
      const resourceDto = defaultResourceFormDto({
        ...
        secret: { object_type: SECRET_DATA_OBJECT_TYPE, custom_fields: [emptyCustomFieldDto()], description: "" },
      });
      const resourceTypeDtos = resourceTypesCollectionDto();
      const resourceTypesCollection = new ResourceTypesCollection(resourceTypeDtos);
  ...
  describe("::removeUnusedNonEmptyMetadata", () => {
    it("should remove the username if the given resource type does not have password", () => {
      ...
      const resourceDto = defaultResourceFormDto({
        resource_type_id: TEST_RESOURCE_TYPE_V5_STANDALONE_NOTE,
        secret: { description: "Here's a description" },
      });
      const resourceTypeDtos = resourceTypesCollectionDto();
      const resourceTypesCollection = new ResourceTypesCollection(resourceTypeDtos);
