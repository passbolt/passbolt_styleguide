...
describe("ResourceSecretRevisionsCollection", () => {
  describe("::getSchema", () => {
    it("schema must validate", () => {
      EntitySchema.validateSchema(
        ResourceSecretRevisionsCollection.name,
        ResourceSecretRevisionsCollection.getSchema(),
      );
    });
  });
  ...
  describe("::constructor", () => {
    ...
    it("should throw if the collection schema does not validate", () => {
      ...
      expect(() => new ResourceSecretRevisionsCollection({})).toThrowEntityValidationError("items");
    });
    ...
    it("should throw if one of data item does not validate the unique id build rule", () => {
      ...
      expect(() => new ResourceSecretRevisionsCollection(dtos)).toThrowCollectionValidationError("1.id.unique");
    });
    ...
    it("should throw if one of resource_id does not validate the collection entity schema", () => {
      ...
      expect(() => new ResourceSecretRevisionsCollection(dtos)).toThrowCollectionValidationError(
        "1.resource_id.different_resource_id",
      );
    });
  ...
  describe("::sortByMostRecent", () => {
    it("should sort by most recent by modified if property is defined", async () => {
      ...
      const recentSecretRevisionRecent = defaultSecretRevisionDto({
        resource_id: resource_id,
        modified: "2025-10-11T08:09:00+00:00",
      });
      ...
      expect(collection.toDto(SecretRevisionEntity.ALL_CONTAIN_OPTIONS)).toEqual([
        recentSecretRevisionRecent,
        oldSecretRevision,
      ]);
    });
  });
