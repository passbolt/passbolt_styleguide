...
describe("OAuth2SsoSettingsFormEntity", () => {
  ...
  describe("::cloneWithMutation", () => {
    it("should return a new instance with the modified field", () => {
      expect.assertions(3);

      const formEntity = new OAuth2SsoSettingsFormEntity(defaultOAuth2SsoSettingsFormEntityDto());
      const cloned = formEntity.cloneWithMutation("client_secret", "new-secret");

      expect(cloned).not.toEqual(formEntity);
      expect(cloned.client_secret).toStrictEqual("new-secret");
      expect(formEntity.client_secret).not.toStrictEqual("new-secret");
    });
  });
