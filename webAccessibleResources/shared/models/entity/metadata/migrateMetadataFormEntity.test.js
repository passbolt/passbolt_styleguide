...
describe("migrateMetadataFormEntity", () => {
  describe("::verifyHealth", () => {
    ...
    it("add an error if there are no V5 resource types to migrate to", () => {
      ...
      expect(healthIssues.getError("migrate_resources_to_v5", "resource_types_v5_deleted")).toStrictEqual(
        "Resource types v5 are deleted.",
      );
    });
    ...
    it("add an error if there are missing V5 resource types to migrate to", () => {
      ...
      expect(healthIssues.getError("migrate_resources_to_v5", "resource_types_v5_partially_deleted")).toStrictEqual(
    ...
    it("add an error if there is are metadata keys but no active ones to migrate with", () => {
      ...
      expect(Object.keys(healthIssues.details)).toHaveLength(1);
      expect(healthIssues.hasError("global_form", "active_metadata_key")).toBeTruthy();
      expect(healthIssues.getError("global_form", "active_metadata_key")).toStrictEqual(
        "No active metadata key defined.",
      );
    });
    ...
    it("add an error if there is are metadata keys but no active ones to migrate with", () => {
      ...
      expect(healthIssues.getError("migrate_resources_to_v5", "allow_creation_of_v5_resources")).toStrictEqual(
