...
class DisplayContentTypesEncryptedMetadataAdministration extends Component {
   ...
   * @return {EntityValidationError}
   */
  verifyDataHealth = memoize((formSettingsDto, resourceTypes, metadataKeysCollection) =>
  ...
  async save() {
    ...
    try {
      ...
      await this.props.actionFeedbackContext.displaySuccess(
        this.props.t("The encrypted metadata settings were updated."),
      );
    } catch (error) {
          ...
          <div className="main-content">
            <form onSubmit={this.handleFormSubmit} data-testid="submit-form">
              ...
              <p className="description">
                <Trans>Encrypted metadata for resources is available.</Trans>{" "}
                <Trans>Define the strategy to manage and migrate the legacy items.</Trans>
              </p>
              ...
              >
                <input
                  ...
                  name="allow_creation_of_v5_resources"
                  id="allowCreationOfV5ResourcesInput"
                  onChange={this.handleInputChange}
                  checked={this.state.settings.allow_creation_of_v5_resources}
                  disabled={this.hasAllInputDisabled()}
                />
                <label htmlFor="allowCreationOfV5ResourcesInput">
                  <span className="name">
                    <Trans>Enable encrypted metadata (recommended)</Trans>
                  </span>
                  <span className="info">
                  ...
                  {errors?.hasError("allow_creation_of_v5_resources", "is_default") && (
                    <div className="name error-message">
                      <Trans>Encrypted metadata must be enabled to set it as the default type.</Trans>
                    </div>
                  )}
                    ...
                    <>
                      {warnings?.hasError("allow_creation_of_v5_resources", "resource_types_deleted") && (
                        <div className="name warning-message">
                          <Trans>
                            All encrypted metadata resource types were previously disabled. Re-enable them if you want
                            users to create resources of this type.
                          </Trans>
                        </div>
                      )}
                      {warnings?.hasError("allow_creation_of_v5_resources", "active_metadata_key") && (
                        <div className="name warning-message">
                          <Trans>
                            A metadata key should be enabled to allow users to create resources of this type.
                          </Trans>
                        </div>
                      )}
                    </>
              ...
              >
                <input
                  type="checkbox"
                  className="toggle-switch-checkbox checkbox"
                  name="allow_creation_of_v4_resources"
                  id="allowCreationOfV4ResourcesInput"
                  onChange={this.handleInputChange}
                  checked={this.state.settings.allow_creation_of_v4_resources}
                  disabled={this.hasAllInputDisabled()}
                />
                <label htmlFor="allowCreationOfV4ResourcesInput">
                  <span className="name">
                    <Trans>Enable legacy cleartext metadata</Trans>
                  </span>
                  <span className="info">
                  ...
                  {errors?.hasError("allow_creation_of_v4_resources", "is_default") && (
                    <div className="name error-message">
                      <Trans>Legacy cleartext metadata must be enabled to set it as the default type.</Trans>
                    </div>
                  )}
                  {!errors?.hasError("allow_creation_of_v4_resources") &&
                    warnings?.hasError("allow_creation_of_v4_resources", "resource_types_deleted") && (
                      <div className="name warning-message">
                        <Trans>
                          All legacy cleartext resource types were previously disabled. Re-enable them if you want users
              ...
              <div className="radiolist-alt">
                ...
                >
                  ...
                  <label htmlFor="defaultResourceTypesV5Input">
                    <span className="name bold">
                      <Trans>Encrypted metadata (recommended)</Trans>
                    </span>
                    <span className="info">
                      <Trans>Users can create resources with encrypted metadata by default.</Trans>
                      <br />
                    </span>
                    ...
                    {!errors?.hasError("default_resource_types", "allow_create_v5") &&
                      warnings?.hasError("default_resource_types", "resource_types_v5_deleted") && (
                        <div className="name warning-message">
                          <Trans>
                            All encrypted metadata resource types were previously disabled. Re-enable them if you want
                            users to create resources of this type.
                ...
                <div
                  className={`input radio ${this.state.settings.default_resource_types === "v4" ? "checked" : ""}
                ...
                >
                  ...
                  <label htmlFor="defaultResourceTypesV4Input">
                    ...
                    {!errors?.hasError("default_resource_types", "allow_create_v4") &&
                      warnings?.hasError("default_resource_types", "resource_types_v4_deleted") && (
                        <div className="name warning-message">
                          <Trans>
                            All legacy cleartext resource types were previously disabled. Re-enable them if you want
                            users to create resources of this type.
                          </Trans>
                        </div>
                      )}
              ...
              <div className="input toggle-switch form-element">
                <input
                  type="checkbox"
                  className="toggle-switch-checkbox checkbox"
                  name="allow_v4_v5_upgrade"
                  id="allowV4V5UpgradeInput"
                  onChange={this.handleInputChange}
                  checked={this.state.settings.allow_v4_v5_upgrade}
                  disabled={this.hasAllInputDisabled()}
                />
                <label htmlFor="allowV4V5UpgradeInput" className="text">
                  ...
                  {warnings?.hasError("allow_v4_v5_upgrade", "resource_types_deleted") && (
                    <div className="name warning-message">
                      <Trans>
                        All encrypted metadata resource types were previously disabled. Re-enable them if you want users
                        to upgrade their resources.
                      </Trans>
                    </div>
                  )}
                  {warnings?.hasError("allow_v4_v5_upgrade", "allow_creation") && (
                    <div className="name warning-message">
                      <Trans>Encrypted metadata should be enabled to allow users to upgrade their resources.</Trans>
                    </div>
                  )}
                </label>
              </div>
        ...
        {createSafePortal(
          <div className="sidebar-help-section">
            ...
            <a
              ...
              href="https://passbolt.com/docs/admin/metadata-encryption/encrypted-metadata/"
            >
              <FileTextSVG />
              <span>
...
export default withAppContext(
  withActionFeedback(
    withDialog(
      withResourceTypesLocalStorage(withTranslation("common")(DisplayContentTypesEncryptedMetadataAdministration)),
    ),
  ),
);
